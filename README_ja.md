...
- **全プロポ対応** — カラー LCD（TX16S, TX18S, Boxer 等）とモノクロ LCD（Zorro, TX12 等）の両方で動作
...
| **VTX** | SmartAudio または Tramp 対応の VTX |
| **レシーバー** | VTX と SmartAudio/Tramp で接続された ELRS レシーバー |

> **補足:** VTX がレシーバーの SmartAudio/Tramp 出力に配線されていて、ELRS 側で VTX Admin が有効になっている必要があります。すでに ELRS Lua → VTX Administrator から VTX の設定を変更できているなら、準備 OK です。

## インストール

**ファイル 1 つだけ。依存なし、設定不要。**

1. [`EasyVTXch.lua`](https://raw.githubusercontent.com/Saqoosha/EasyVTXch/main/EasyVTXch.lua) をダウンロード
2. プロポの SD カードにコピー：
   ```
...
- **EdgeTX Companion:** SD カード同期機能を利用
...
### カラー LCD プロポ（TX16S, TX18S, Boxer 等）
...
| チャンネルは変わるが VTX が反応しない | レシーバーと VTX 間の SmartAudio/Tramp 配線を確認。VTX がそのプロトコルに対応しているかも確認してください。 |
